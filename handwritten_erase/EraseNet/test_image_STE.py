...
result_with_mask = os.path.join(savePath, 'WithMaskOutput/') # savePath + 'WithMaskOutput/'
result_straight = os.path.join(savePath, 'StrOuput/') # savePath + 'StrOuput/'
...
for imgs, gt, masks, path in (Erase_data):
    ...
    save_image(g_image_with_mask, os.path.join(result_with_mask, filename))
    save_image(g_image, os.path.join(result_straight, filename))
